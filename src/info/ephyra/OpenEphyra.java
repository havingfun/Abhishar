...
//               Search.addKnowledgeMiner(new IndriKM());
                //System.out.println("loading Indri");
...
//			Search.addKnowledgeMiner(new IndriKM(indriServers, true));
	 ...
	 */
        ...
        public String questionAnalyse(String doc,String question)
        {
			...
			if (question.matches("(?i)" + FACTOID + ":.*+")) {
				...
				type = FACTOID;  // default type
                        ...
                        AnalyzedQuestion aq = analyzer(question);
                        Query[] queries = queryStrings(aq);
                        Result[] rs = new Result[queries.length];
                        String ar[] = {"Answer type testing","Pattern learning and matching","Semantic parsing"};
                        for(int i=0;i<queries.length;i++)
                        {
                            queries[i].setExtractionTechniques(ar);
                            rs[i] = new Result(doc,queries[i]);
                            rs[i].setScore(Float.NEGATIVE_INFINITY);
                        }


                        AnswerTypeFilter atf = new AnswerTypeFilter();
                        rs = atf.apply(rs);
                        for(int i=queries.length;i<rs.length;i++)
                        {
                            System.out.println(rs[i].getAnswer());
                        }
                        AnswerPatternFilter apf = new AnswerPatternFilter();
                        rs = apf.apply(rs);
                        for(int i=queries.length;i<rs.length;i++)
                        {
                            System.out.println(rs[i].getAnswer());
                        ...
                        rs = AnswerSelection.getResults(rs, FACTOID_MAX_ANSWERS, LIST_REL_THRESH);

                        if(rs.length==1)return rs[0].getAnswer();
                        else
                            return "Anser Not Found";
                        
        }

        public Result[] answerType(String doc,String question)
        {
                        AnalyzedQuestion aq = analyzer(question);
                        Query[] queries = queryStrings(aq);
                        Result[] rs = new Result[queries.length];
                        String techniques[] = {"Answer type testing"};
                        for(int i=0;i<queries.length;i++)
                        {
                            queries[i].setExtractionTechniques(techniques);
                            rs[i] = new Result(doc,queries[i]);
                            rs[i].setScore(Float.NEGATIVE_INFINITY);
                        }
                        AnswerTypeFilter atf = new AnswerTypeFilter();
        ...
        public Result[] answerType(Result[] rs)
        {
                        AnswerTypeFilter atf = new AnswerTypeFilter();
                        rs = atf.apply(rs);
                        return rs;
        ...
        public Result[] answerPattern(String doc,String question)
        {
                        AnalyzedQuestion aq = analyzer(question);
                        Query[] queries = queryStrings(aq);
                        Result[] rs = new Result[queries.length];
                        String techniques[] = {"Pattern learning and matching"};
                        for(int i=0;i<queries.length;i++)
                        {
                            queries[i].setExtractionTechniques(techniques);
                            rs[i] = new Result(doc,queries[i]);
        ...
        public Result[] answerPattern(Result[] rs)
        {
                        AnswerPatternFilter atf = new AnswerPatternFilter();
                        rs = atf.apply(rs);
                        return rs;
        }
	...
	public void commandLine() {
		while (true) {
			...
			MsgPrinter.printAnswers(results);
                        ...
                        {
                            queries[i].setExtractionTechniques(ar);
                            rs[i] = new Result("Bill Gates is an American Business Tycoon. India is on a Moon.",queries[i]);
                            rs[i].setScore(Float.NEGATIVE_INFINITY);
                        }
	 ...
	 * @return array of results
	 */
        public AnalyzedQuestion analyzer(String question)
        {
            AnalyzedQuestion aq = QuestionAnalysis.analyze(question);
            return aq;
        }

        public Query[] queryStrings(AnalyzedQuestion aq)
        {
            Query[] queries = QueryGeneration.getQueries(aq);
            return queries;
        }
