...
public class Main {
    public static Bot[] bot = new Bot[5];
    public static Chat[] chatSession = new Chat[5];
    public static void main (String[] args,int botid) {
        MagicStrings.root_path = System.getProperty("user.dir");
        System.out.println("Working Directory = " + MagicStrings.root_path);
        AIMLProcessor.extension =  new PCAIMLProcessorExtension();
        mainFunction(args,botid);
    }
    public static void load(String[] args,int botid)
    {
        MagicStrings.root_path = System.getProperty("user.dir");
        System.out.println("Working Directory = " + MagicStrings.root_path);
        AIMLProcessor.extension =  new PCAIMLProcessorExtension();
    ...
    public static void mainFunction (String[] args,int botid) {
        String botName = "abhishar";
        String action = "chat";
        ...
        for (String s : args) {
            ...
            if (splitArg.length >= 2) {
                ...
                if (option.equals("trace") && value.equals("true")) MagicBooleans.trace_mode = false;
        ...
        bot[botid] = new Bot(botName, MagicStrings.root_path, action); //
        //bot.preProcessor.normalizeFile("c:/ab/log1.txt", "c:/ab/data/lognormal.txt");
        if (bot[botid].brain.getCategories().size() < 100) bot[botid].brain.printgraph();
        chatSession[botid] = new Chat(bot[botid]);
        //bot[botid].brain.nodeStats();
       // System.out.println(botid);
        //testChat(bot, true);
        /*if (action.equals("chat")) processChat("Hello");
    ...
    public static String processChat (String textLine,int id) {

        MagicBooleans.trace_mode = false;
        if (textLine == null || textLine.length() < 1)  textLine = MagicStrings.null_input;
        String request = textLine;
        if (MagicBooleans.trace_mode) System.out.println("STATE="+request+":THAT="+chatSession[id].thatHistory.get(0).get(0)+":TOPIC="+chatSession[id].predicates.get("topic"));
        String response = chatSession[id].multisentenceRespond(request);
        while (response.contains("&lt;")) response = response.replace("&lt;","<");
        while (response.contains("&gt;")) response = response.replace("&gt;",">");
        System.out.println("Robot: "+response);
        return response;
